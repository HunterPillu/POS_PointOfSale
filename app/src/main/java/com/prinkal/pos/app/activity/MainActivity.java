...
public class MainActivity extends BaseActivity {
    ...
    private void reminderMsg() {
        if (!AppSharedPref.isReminderMsgShown(this, false)) {
            ...
            sweetAlert.setTitleText(getString(R.string.warning)).setContentText(getResources().getString(R.string.demo_app_popup)).setConfirmText(getResources().getString(R.string.never)).setConfirmClickListener(new SweetAlertDialog.OnSweetClickListener() {
                @Override
                public void onClick(SweetAlertDialog sDialog) {
                    sDialog.dismissWithAnimation();
                    AppSharedPref.setReminderMsgShown(MainActivity.this, true);
                }
            }).setCancelText(getResources().getString(R.string.remind_me_later)).setCancelClickListener(new SweetAlertDialog.OnSweetClickListener() {
                @Override
                public void onClick(SweetAlertDialog sDialog) {
                    sDialog.dismissWithAnimation();
                }
    ...
    class GetNetworkTime extends CoroutinesAsyncTask<Void, Void, Long> {
    ...
    private void initDrawerToggle() {
        mDrawerToggle = new ActionBarDrawerToggle(this, mMainBinding.drawerLayout, R.string.drawer_open, R.string.drawer_close);
        mMainBinding.drawerLayout.addDrawerListener(mDrawerToggle);
    }

    private void initBottomNavView() {
        mMainBinding.bottomNavView.setOnNavigationItemSelectedListener(new BottomNavigationView.OnNavigationItemSelectedListener() {
            @Override
    ...
    private void loadHomeFragment() {
        ...
        fragmentTransaction.replace(R.id.main_frame, HomeFragment.newInstance(), HomeFragment.class.getSimpleName());
        fragmentTransaction.commit();
    }

    private void loadOrdersFragment() {
        FragmentTransaction fragmentTransaction = getSupportFragmentManager().beginTransaction();
        fragmentTransaction.replace(R.id.main_frame, OrdersFragment.newInstance(), OrdersFragment.class.getSimpleName());
        fragmentTransaction.commit();
    }

    private void loadHoldFragment() {
        FragmentTransaction fragmentTransaction = getSupportFragmentManager().beginTransaction();
        fragmentTransaction.replace(R.id.main_frame, HoldFragment.newInstance(), HoldFragment.class.getSimpleName());
        fragmentTransaction.commit();
    }

    private void loadMoreFragment() {
        FragmentTransaction fragmentTransaction = getSupportFragmentManager().beginTransaction();
        fragmentTransaction.replace(R.id.main_frame, MoreFragment.newInstance(), MoreFragment.class.getSimpleName());
        fragmentTransaction.commit();
    }
    ...
    public void removeBottomNavShiftMode() {
        try {
            BottomNavigationMenuView menuView = (BottomNavigationMenuView) mMainBinding.bottomNavView.getChildAt(0);
    ...
    public void onBackPressed() {
        if (mMainBinding.drawerLayout != null && mMainBinding.drawerLayout.isDrawerOpen(GravityCompat.START)) {
